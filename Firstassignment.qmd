...
dfmid <- dfmid %>% select(Globe) %>% 
  mutate_if(is.character, ~as.numeric(.)) %>%
  rename(mid_t = Globe)

dftropo <- dftropo %>% select(Globe) %>% 
  mutate_if(is.character, ~as.numeric(.)) %>%
  rename(tropo = Globe)

dfstrato <- dfstrato %>% select(Globe) %>% 
  mutate_if(is.character, ~as.numeric(.)) %>%
...
df_long %>% filter(Year >= 1980) %>% 
  ...
  geom_line(aes(x=Date, y=Average.Temp,col="#0072B2"),size=1.4)+
  ...
  labs(title="Temperature over time", x="Year",y="Global Temperature")+
    ...
    scale_color_manual(name="Location", values=c("#0072B2"
    ,"orange"
    ,"brown",
    "cornflowerblue",
    "turquoise"), 
